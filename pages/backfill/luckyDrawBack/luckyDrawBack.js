...
Page({
  ...
  successMusic(states) {
    ...
    //   })
    //   mbps[0].bgm.types = 'success'
    //   mbps[0].bgm.color = '#FF6B4E'
    //   this.arrayMandas.staging = mbps[0].bgm.name
    //   this.arrayMandas.stagingIds = mbps[0].bgm.id
  ...
  choiseTles() { //标题选择
    ...
    Create.sloganChoose(this, 'LOTTERY')
  },
  ...
  pushLoadimg(data) {
    var that = this, pics = that.data.mainf,
      i = data.i ? data.i : 0,
  ...
  tooFalse() {
    ...
    let Drl = this.Haha(this.data.picsBox), prize = { trueArr: [], falseArr: [] }
    let proba = 0
    this.data.awardList.map((item) => {
      if (item.awardControl == false) {
        prize.falseArr.push(item)
        if (item.remainQty != null && item.subject && item.probability) {
    ...
    let param = {
      'activityBase': {
        ...
        'actPics': Drl, //活动图片
        'actType': this.data.pageData.luckyDrawActModel.luckyDrawAct.type,
      ...
      'luckyDrawActModel': {
        'luckyDrawAct': {
          'countType': 'EVERY_DAY',
          'count': this.data.pageData.luckyDrawActModel.luckyDrawAct.count,
          'awardCount': this.data.pageData.luckyDrawActModel.luckyDrawAct.awardCount,
          //'productName': this.data.merchantName,商品名称
          'type': this.data.pageData.luckyDrawActModel.luckyDrawAct.type,//活动类型
        },
        'luckyDrawPrizes': [] //数组
      }
    }
    ...
    if (massage != 'ok') {
      ...
      if (okay) {
        if (proba > 100) { console.log(proba),Create.prompt('中奖概率需小于等于100%') }else{
          if (prize.trueArr.length == prize.falseArr.length){
            prize.trueArr.map((items) => { items.probability = items.probability / 100 })
            param.luckyDrawActModel.luckyDrawPrizes = prize.trueArr
  ...
  setEditor(states) {  //数据成功回调
    ...
    if (modify.bizCode == 200) {
      ...
      // let Ctr = this.setVariable(modify.data.activityBase.mainPic)
      let Drl = this.setVariable(modify.data.activityBase.actPics)
      ...
      let rt = this.dealWith(modify.data.luckyDrawActModel.luckyDrawPrizes)
      this.setData
        ({
  ...
  dealWith(act){
    ...
    if(Arr.length<7){
      let ad = 7-Arr.length
      for(let i=0;i<ad;i++){
        Arr.push({ awardControl: true, subject: '', remainQty: '', probability: '' })
      }
    }
    Arr.map((item) => { item.probability = Create.mathSign(item.probability * 100) })
    return Arr
  },
  /**
   * 生命周期函数--监听页面加载
