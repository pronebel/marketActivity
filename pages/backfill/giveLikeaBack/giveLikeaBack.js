    ...
    setTimeout(function(){
      ...
      let param = {
        ...
        'assistanceAct': {
          ...
          'helpCount': that.data.giveLike * 100,
      ...
      let inputArr = [
        ...
        { dataSource: that.data.merchantAddres, errMassage: '店铺名称不能为空' },
      ...
      if (massage != 'ok') {
        ...
        if (okay) {
          if (that.data.giveLike * 100 > that.data.largestNum * 100) {
            console.log(param)
            if (that.data.largestNum * 100 > that.data.miniNum * 100) {
              ajaxhelper.post(app.globalData.frontJSHost + 'activityBase/update', param, that, that.pushUp)
            } else {
              Create.prompt('请正确设置价格区间')
            }
          } else {
            Create.prompt('最大现金值应大于价格区间最大值')
          }
        }
      }
    },100)
