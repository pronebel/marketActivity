...
Page({
  ...
  setListData(res) {
    ...
    if(res.status){
      ...
      this.setData({
        ...
        presentPrice: buyAct.currentPrice / 100,
      ...
      activityBase.bgmName && this.setData({isMusic: activityBase.bgmName})
  ...
  choiseTles() { //标题选择
    ...
    Create.sloganChoose(this, 'NYUANBUY')
  },
  ...
  // formSubmit(e) {
  //   this.data.activityRule = e.detail.value.activityRule;
  //   console.log(this.data.activityRule,11)
  // },
  tooFalse(e) {
    let mandnames = '', drl = '';
    ...
    setTimeout(function () {
      let param = {
        ...
        'buyAct': {
          ...
          'currentPrice': that.data.presentPrice*100,  //现价
          'remainQty': that.data.merchantNum * 1,// 商品数量
          'maxBuyCount': that.data.largestNum * 1,
          'minBuyCount': that.data.miniNum * 1,
      ...
      let inputArr = [
        ...
        { dataSource: that.data.presentPrice, errMassage: '现价不能为空' },
        { dataSource: that.data.largestNum, errMassage: '最大数量不能为空' },
        { dataSource: that.data.miniNum, errMassage: '最少数量不能为空' }
      ]
      ...
      if (massage != 'ok') {
        ...
        if (okay) {
          if (that.data.originPrice * 100 > that.data.presentPrice * 100) {
            if (that.data.largestNum * 1 > that.data.miniNum * 1) {
            ajaxhelper.post(app.globalData.frontJSHost + 'activityBase/update', param, that, that.pushUp)
            } else {
              Create.prompt('最大数量需大于最小数量')
            }
          } else {
            Create.prompt('现价需低于原价')
          }
        }
      }
    },100)
