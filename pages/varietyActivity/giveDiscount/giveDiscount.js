...
Page({
  ...
  nowPrice(e) { this.setData({ nowPrice: e.detail.value }) },
  giveLike(e) { this.setData({ giveLike: e.detail.value }) },
  ...
  tooFalse() {
    ...
    let param = {
      'activityBase': {
        ...
        'actType': 'ASSISTANCE_DISCOUNT'
      },
      'assistanceAct': {
        'currentPrice': this.data.nowPrice*100,//现价
        'maxStockCount': this.data.merchantNum * 1,
        'originPrice': this.data.originPrice * 100,//原价
        'helpCount': this.data.giveLike * 1,
        'productName': this.data.merchantName,//商品名称
        'maxEveryHelp': 1,
        'minEveryHelp': 1,
        'type': 'WINNING_DISCOUNT'
      }
    }
    let inputArr = [
      ...
      { dataSource: this.data.nowPrice, errMassage: '现价不能为空' },
      { dataSource: this.data.giveLike, errMassage: '需要点赞数量不能为空' },
    ]
    ...
    if (massage != 'ok') {
      ...
      if (this.data.nowPrice*1>0){
        if (this.data.originPrice*1>this.data.nowPrice * 1){
          if (okay) {
            if (this.data.giveLike * 1 > 0) {
              ajaxhelper.post(app.globalData.frontJSHost + 'activityBase/add', param, this, this.pushUp)
            } else { Create.prompt('点赞数需大于零') }
          }
        } else { Create.prompt('原价不能小于现价') }
      } else { Create.prompt('现价不能小于0') }
    }
  },
  pushUp(states) {
