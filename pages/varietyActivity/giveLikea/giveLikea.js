...
Page({
  ...
  tooFalse() {
    ...
    let param = {
      ...
      'assistanceAct': {
        'helpCount': this.data.giveLike * 100,
        'maxEveryHelp': this.data.largestNum*100,
        'minEveryHelp': this.data.miniNum * 100,
        'type': 'WINNING_MONEY'
      }
    }
    let inputArr = [
      ...
      { dataSource: this.data.largestNum, errMassage: '最大价格不能为空' },
      { dataSource: this.data.miniNum, errMassage: '最小价格不能为空' },
    ...
    if (massage != 'ok') {
      ...
      if (okay) {
        if (this.data.giveLike * 100 > this.data.largestNum * 100) {
          console.log(param)
          if (this.data.largestNum * 100 > this.data.miniNum * 100) {
            ajaxhelper.post(app.globalData.frontJSHost + 'activityBase/add', param, this, this.pushUp)
          } else {
            Create.prompt('请正确设置价格区间')
          }
        } else {
          Create.prompt('最高金额值应大于价格区间最大值')
        }
      }
    }
  },
