...
Page({
  ...
  tooFalse() {
    ...
    let param = {
      ...
      'seckillAct': {
        'currentPrice': this.data.presentPrice*100,//现价
        'originPrice': this.data.originPrice * 100,//原价
        'maxStockCount': this.data.merchantNum*1,// 商品数量
        'maxEveryoneBuyCount': this.data.largestNum*1, //每人最大购买数量
        'minEveryoneBuyCount': this.data.miniNum*1, //每人最小购买数量
    ...
    let inputArr = [
      ...
      { dataSource: this.data.presentPrice, errMassage: '现价不能为空' },
      { dataSource: this.data.largestNum, errMassage: '最大购买数量不能设置空' },
      { dataSource: this.data.miniNum, errMassage: '最小购买数量不能设置空' }
    ]
    ...
    if (massage != 'ok') {
      ...
      if (okay) {
        if (this.data.miniNum * 1 > 0){
          if (this.data.presentPrice * 100 > 0){
            if (this.data.originPrice * 100 > this.data.presentPrice * 100) {
              if (this.data.largestNum*1 > this.data.miniNum*1){
                ...
                } else { Create.prompt('最大购买数量不能超过商品数量')}
              }else {
            ...
            Create.prompt('现价不能小于0')
          }
        }else { Create.prompt('最小购买数量不能低于0') }
      }
    }
  },
  pushUp(states) {
