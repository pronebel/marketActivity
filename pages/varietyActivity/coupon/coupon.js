...
Page({
  ...
  data: {
    ...
    musicidx: 0,
    awardList: [
      { reductions: true, maxStockCount: '', discount: '', conditions: '' },
      { reductions: true, maxStockCount: '', discount: '', conditions: '' },
      { reductions: true, maxStockCount: '', discount: '', conditions: '' },
      { reductions: true, maxStockCount: '', discount: '', conditions: '' },
      { reductions: true, maxStockCount: '', discount: '', conditions: '' },
  ...
  pageTrue() { 
    this.data.paybackList[this.arrayMandas.juancless].delFlag = true
    this.setData({
      paybacked: false,
      pagei: this.arrayMandas.juancless,
      Secur: this.arrayMandas.juanname,
      securType: this.arrayMandas.juantype,
      paybackList: this.data.paybackList
    })
  },
  ...
  shutDown() { this.setData({ paybacked: false }) },
  ...
  friends(ad){
    let that = ad, prize = { trueArr: [], falseArr: [] }
    that.data.awardList.map((item) => {
      if (item.reductions == false) {
        price.falseArr.push(item)
        if (item.maxStockCount && item.discount && item.conditions) {
          item.type = this.data.securType,
          item.name = this.data.Secur,
  ...
  tooFalse() {
    ...
    let param = {
      ...
      "quanActModel": {
        ...
        "quanList": [{
          "reductions": false,
          'maxStockCount': this.data.securitiesNum,
          'discount': this.data.denomination,
          'conditions': '无门槛使用',
          "type": this.data.securType,
          "name": this.data.Secur,
          'validiteDate': this.data.validity,
        }]
      }
    }
    let inputArr = [
    ...
    var prize = this.friends(this)
    if (this.data.Secur != '红包劵') {
      param.quanActModel.quanList = prize.trueArr
    }else {
        ...
        if (prize.falseArr.length>0){
          if (prize.trueArr.length == prize.falseArr.length){
            if (timestampend < validity) {
              ajaxhelper.post(app.globalData.frontJSHost + 'activityBase/add', param, this, this.pushUp)
            } else {
              Create.prompt('有效期需大于活动结束时间')
            }
          }else { Create.prompt('打开的优惠劵需填写完整') }
        } else { Create.prompt('最少需填写一个优惠劵') }
      }
    }
  },
  pushUp(states) {
  ...
  onLoad: function (options) {
    ...
    var val = thisdate*1 + 691200000
    var endtime = new Date(parseInt(weekdate)).format('yyyy-MM-dd'), validity = new Date(parseInt(val)).format('yyyy-MM-dd')
